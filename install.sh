...
if [ "$INSTALL_MODE" = "4" ]; then
    ...
    WP_TMP=$(mktemp -d)
    git clone --depth=1 https://github.com/mylinuxforwork/wallpaper.git "$WP_TMP/ml4w-wallpapers"

    mkdir -p ~/Wallpapers/{manha,tarde,noite}

    mapfile -t WALLPAPERS < <(find "$WP_TMP/ml4w-wallpapers" -type f \( -iname "*.jpg" -o -iname "*.png" -o -iname "*.jpeg" \))
    TOTAL=${#WALLPAPERS[@]}
    PER=$((TOTAL / 3))
    ...
    for ((i = 0; i < TOTAL; i++)); do
        f="${WALLPAPERS[$i]}"
        if [ $i -lt $PER ]; then
            cp "$f" ~/Wallpapers/manha/
        elif [ $i -lt $((PER * 2)) ]; then
            cp "$f" ~/Wallpapers/tarde/
        else
            cp "$f" ~/Wallpapers/noite/
        fi
    done
...
if command -v yay &>/dev/null; then
    AUR_HELPER="yay"
    success "yay detectado"
elif command -v paru &>/dev/null; then
    AUR_HELPER="paru"
    success "paru detectado"
else
...
if ask "Baixar e organizar wallpapers do ML4W?"; then
    ...
    WP_TMP2=$(mktemp -d)
    git clone --depth=1 https://github.com/mylinuxforwork/wallpaper.git "$WP_TMP2/ml4w-wallpapers"

    mkdir -p ~/Wallpapers/{manha,tarde,noite}

    mapfile -t WALLPAPERS < <(find "$WP_TMP2/ml4w-wallpapers" -type f \( -iname "*.jpg" -o -iname "*.png" -o -iname "*.jpeg" \))
    TOTAL=${#WALLPAPERS[@]}
    PER=$((TOTAL / 3))
